...
import { CONFLICT_OUTPUT_FILE, DIFF_VIEW_CONFIG, SPLIT_DIFF_VIEW_CONFIG } from "./constants";
...
export default class Tools {
    ...
    async hasTooBigFiles(files: { vaultPath: string; path: string }[]): Promise<boolean> {
        ...
        if (remoteUrl?.includes("github.com")) {
            ...
            for (const f of files) {
                const file = this.plugin.app.vault.getAbstractFileByPath(f.vaultPath);
                let over100mb = false;
                ...
                if (file instanceof TFile) {
                    ...
                    const statRes = await this.plugin.app.vault.adapter.stat(f.vaultPath);
                ...
                if (over100mb) {
                    ...
                    if (gitManager instanceof SimpleGit) {
                        isFileTrackedByLfs = await gitManager.isFileTrackedByLFS(f.path);
                    }
    ...
    async writeAndOpenFile(text?: string) {
        if (text !== undefined) {
            await this.plugin.app.vault.adapter.write(CONFLICT_OUTPUT_FILE, text);
        }
        let fileIsAlreadyOpened = false;
        this.plugin.app.workspace.iterateAllLeaves((leaf) => {
            if (leaf.getDisplayText() != "" && CONFLICT_OUTPUT_FILE.startsWith(leaf.getDisplayText())) {
                fileIsAlreadyOpened = true;
            }
        });
        if (!fileIsAlreadyOpened) {
            await this.plugin.app.workspace.openLinkText(CONFLICT_OUTPUT_FILE, "/", true);
        }
    }
